...
export function validateScenario(input: unknown): asserts input is ScenarioV1 {
  ...
  for (const monster of scenario.monsters ?? []) {
    ...
    if (!Array.isArray(monster.tile) || monster.tile.length !== 2)
      throw new Error("Monster tile must be [x, y]");
    if (monster.id && ids.has(monster.id))
      throw new Error(`Duplicate entity id: ${monster.id}`);
  ...
  const state: GameState = {
    ...
    player: {
      ...
      facing: input.player?.facing
        ? { x: input.player.facing[0], y: input.player.facing[1] }
        : { x: 1024, y: 0 },
      radius: 320,
