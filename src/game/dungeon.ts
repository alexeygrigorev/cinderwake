...
function carveRoom(tiles: number[], mapWidth: number, room: Room): void {
  for (let y = room.y; y < room.y + room.height; y += 1) {
    for (let x = room.x; x < room.x + room.width; x += 1) tiles[y * mapWidth + x] = 0;
  }
}

function carveCorridor(tiles: number[], mapWidth: number, from: Vec2, to: Vec2, horizontalFirst: boolean): void {
...
export function mapDigest(map: Pick<DungeonMap, 'width' | 'height' | 'tiles' | 'spawn' | 'exit'>): string {
...
export function generateDungeon(seed: string, width = 44, height = 32): DungeonMap {
  const rng = createRng(`${seed}:dungeon-v1`);
  const tiles = new Array<number>(width * height).fill(1);
  const rooms: Room[] = [];
  const first: Room = { x: Math.floor(width / 2) - 4, y: Math.floor(height / 2) - 3, width: 9, height: 7 };
  rooms.push(first);
  ...
  for (let index = 1; index < rooms.length; index += 1) {
    carveCorridor(tiles, width, center(rooms[index - 1]!), center(rooms[index]!), randomInt(rng, 0, 2) === 0);
  }

  const spawn = center(first);
  const exit = rooms
    .map(center)
    .sort((a, b) => Math.abs(b.x - spawn.x) + Math.abs(b.y - spawn.y) - (Math.abs(a.x - spawn.x) + Math.abs(a.y - spawn.y)))[0] ?? spawn;
  const map: DungeonMap = { width, height, tiles, spawn, exit, rooms, digest: '' };
  map.digest = mapDigest(map);
  return map;
}

export function explicitDungeon(rows: string[]): DungeonMap {
  if (rows.length < 3 || !rows[0]) throw new Error('Explicit maps need at least three non-empty rows');
  const width = rows[0].length;
  if (rows.some((row) => row.length !== width)) throw new Error('Every explicit map row must have equal width');
  ...
  if (!spawn || !exit) throw new Error('Explicit maps require exactly one P spawn and one E exit');
  const map: DungeonMap = { width, height: rows.length, tiles, spawn, exit, rooms: [], digest: '' };
  map.digest = mapDigest(map);
  return map;
}

export function tileCenter(tile: Vec2): Vec2 {
  return { x: tile.x * UNITS_PER_TILE + UNITS_PER_TILE / 2, y: tile.y * UNITS_PER_TILE + UNITS_PER_TILE / 2 };
}

export function isFloor(map: DungeonMap, tileX: number, tileY: number): boolean {
