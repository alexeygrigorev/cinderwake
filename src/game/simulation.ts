...
function normalized(from: Vec2, to: Vec2, scale = DIRECTION_SCALE): Vec2 {
  ...
  return { x: Math.round((dx / length) * scale), y: Math.round((dy / length) * scale) };
}

function pointWalkable(state: GameState, point: Vec2, radius: number): boolean {
  const offsets = [[-radius, -radius], [radius, -radius], [-radius, radius], [radius, radius]] as const;
  return offsets.every(([dx, dy]) => isFloor(state.map, Math.floor((point.x + dx) / UNITS_PER_TILE), Math.floor((point.y + dy) / UNITS_PER_TILE)));
}

function moveActor(state: GameState, position: Vec2, velocity: Vec2, radius: number): Vec2 {
  let next = { ...position };
...
function setAnimation(actor: { animation: { clip: AnimationClip; startedAtTick: number; lockedUntilTick: number } }, clip: AnimationClip, tick: number, lock = 0): void {
  if (actor.animation.clip !== clip || clip === 'attack' || clip === 'ability' || clip === 'hurt') {
    actor.animation = { clip, startedAtTick: tick, lockedUntilTick: tick + lock };
  }
}

function applyDamageToMonster(state: GameState, monster: MonsterState, rawDamage: number, sourceId: string): void {
  const damage = Math.max(1, Math.floor((rawDamage * 100) / (100 + monster.armor)));
  monster.health -= damage;
  state.metrics.damageDealt += damage;
  setAnimation(monster, monster.health <= 0 ? 'death' : 'hurt', state.tick, monster.health <= 0 ? 48 : 8);
  emit(state, { type: 'damage', sourceId, targetId: monster.id, amount: damage });
}
...
function spawnLoot(state: GameState, monster: MonsterState): void {
  ...
  const rarity = rarityRoll < 0.7 ? 'common' : rarityRoll < 0.95 ? 'tempered' : 'relic';
  const amount = kind === 'gold' ? (rarity === 'relic' ? 18 : rarity === 'tempered' ? 10 : 6) : kind === 'weapon' ? (rarity === 'relic' ? 6 : rarity === 'tempered' ? 3 : 1) : 1;
  ...
  emit(state, { type: 'loot_dropped', sourceId: monster.id, targetId: loot.id, amount, detail: `${rarity} ${kind}` });
}

function resolveDeaths(state: GameState): void {
  const dead = state.monsters.filter((monster) => monster.health <= 0).sort((a, b) => a.id.localeCompare(b.id));
  for (const monster of dead) {
    state.metrics.kills += 1;
    state.player.xp += MONSTERS[monster.kind].xp;
    emit(state, { type: 'monster_died', sourceId: 'player', targetId: monster.id });
    spawnLoot(state, monster);
  }
  if (dead.length > 0) state.monsters = state.monsters.filter((monster) => monster.health > 0);
...
function hitCone(state: GameState, attack: PendingAttack, threshold: number): void {
  const rangeSquared = attack.range * attack.range;
  for (const monster of [...state.monsters].sort((a, b) => a.id.localeCompare(b.id))) {
    if (distanceSquared(attack.origin, monster.position) > rangeSquared) continue;
    const direction = normalized(attack.origin, monster.position);
    const dot = (direction.x * attack.direction.x + direction.y * attack.direction.y) / DIRECTION_SCALE;
    if (dot >= threshold) applyDamageToMonster(state, monster, attack.damage, attack.ownerId);
  }
}

function createProjectile(state: GameState, attack: PendingAttack, direction = attack.direction, pierce = 0): ProjectileState {
  ...
  const projectile: ProjectileState = {
    ...
    velocity: { x: Math.round((direction.x * speed) / DIRECTION_SCALE), y: Math.round((direction.y * speed) / DIRECTION_SCALE) },
...
function resolvePendingAttacks(state: GameState): void {
  const due = state.pendingAttacks.filter((attack) => attack.impactTick <= state.tick).sort((a, b) => a.id.localeCompare(b.id));
  state.pendingAttacks = state.pendingAttacks.filter((attack) => attack.impactTick > state.tick);
    ...
    if (classId === 'vanguard') {
      ...
      if (attack.kind === 'ability') {
        state.projectiles.push(
          createProjectile(state, attack, rotateDirection(attack.direction, -0.18), 2),
      ...
      for (const monster of state.monsters) {
        if (distanceSquared(state.player.position, monster.position) <= attack.range * attack.range) {
...
function queuePlayerAttack(state: GameState, kind: 'primary' | 'ability'): void {
  const definition = ARCHETYPES[state.player.classId];
  const isAbility = kind === 'ability';
  const impactDelay = state.player.classId === 'ranger' ? (isAbility ? 10 : 6) : isAbility ? 12 : 8;
  const range = isAbility
    ? state.player.classId === 'vanguard' ? 2355 : state.player.classId === 'arcanist' ? 2816 : definition.attackRange
  ...
  state.pendingAttacks.push({
    ...
    range,
    damage: (isAbility ? state.player.abilityDamage : state.player.attackDamage) + state.player.power,
  });
  state.nextEntityId += 1;
  setAnimation(state.player, isAbility ? 'ability' : 'attack', state.tick, isAbility ? 36 : 26);
  emit(state, { type: isAbility ? 'ability_started' : 'attack_started', sourceId: 'player', detail: state.player.classId });
  if (isAbility) state.player.abilityReadyTick = state.tick + definition.abilityCooldown;
...
function updatePlayer(state: GameState, input: InputState): void {
  ...
  else if (moveX !== 0 || moveY !== 0) player.facing = normalized({ x: 0, y: 0 }, { x: moveX, y: moveY });
  player.position = moveActor(state, player.position, player.velocity, player.radius);
  state.metrics.distanceUnits += Math.round(Math.hypot(player.position.x - player.previousPosition.x, player.position.y - player.previousPosition.y));

  if (input.useTonic && player.tonics > 0 && player.health < player.maxHealth) {
    player.health = Math.min(player.maxHealth, player.health + 45);
    player.tonics -= 1;
  }
  if (input.attack && state.tick >= player.attackReadyTick) queuePlayerAttack(state, 'primary');
  else if (input.ability && state.tick >= player.abilityReadyTick) queuePlayerAttack(state, 'ability');
  else if (state.tick >= player.animation.lockedUntilTick) setAnimation(player, moveX !== 0 || moveY !== 0 ? 'walk' : 'idle', state.tick);
}

function damagePlayer(state: GameState, rawDamage: number, sourceId: string): void {
  if (state.tick < state.player.invulnerableUntilTick || state.phase !== 'playing') return;
  const damage = Math.max(1, Math.floor((rawDamage * 100) / (100 + state.player.armor)));
  state.player.health -= damage;
  state.metrics.damageTaken += damage;
  emit(state, { type: 'player_damaged', sourceId, targetId: 'player', amount: damage });
...
function updateMonsters(state: GameState): void {
  ...
  for (const monster of [...state.monsters].sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    const distance = Math.sqrt(distanceSquared(monster.position, state.player.position));
    ...
    if (monster.kind === 'hexer' && distance < 3 * UNITS_PER_TILE) {
      monster.velocity = { x: -Math.round((direction.x * monster.moveSpeed) / 1024), y: -Math.round((direction.y * monster.moveSpeed) / 1024) };
    } else if (distance > monster.attackRange * 0.85 && distance < 9 * UNITS_PER_TILE) {
      monster.velocity = { x: Math.round((direction.x * monster.moveSpeed) / 1024), y: Math.round((direction.y * monster.moveSpeed) / 1024) };
    }
    monster.position = moveActor(state, monster.position, monster.velocity, monster.radius);
    if (distance <= monster.attackRange && state.tick >= monster.attackReadyTick) {
      ...
      if (monster.kind === 'hexer') {
        state.projectiles.push({
          ...
          velocity: { x: Math.round((direction.x * 120) / 1024), y: Math.round((direction.y * 120) / 1024) },
          radius: 130,
      ...
      setAnimation(monster, monster.velocity.x !== 0 || monster.velocity.y !== 0 ? 'walk' : 'idle', state.tick);
    }
  }
}
...
function updateProjectiles(state: GameState): void {
  ...
  for (const projectile of [...state.projectiles].sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    if (state.tick >= projectile.expiresAtTick || !isFloor(state.map, tileX, tileY)) continue;
    let consumed = false;
    if (projectile.hostile) {
      const radius = projectile.radius + state.player.radius;
      if (distanceSquared(projectile.position, state.player.position) <= radius * radius) {
      ...
      for (const monster of [...state.monsters].sort((a, b) => a.id.localeCompare(b.id))) {
        ...
        if (distanceSquared(projectile.position, monster.position) <= radius * radius) {
...
function collectLoot(state: GameState): void {
  ...
  for (const loot of state.loot) {
    ...
    emit(state, { type: 'loot_picked', sourceId: 'player', targetId: loot.id, amount: loot.amount, detail: loot.kind });
  }
...
export function stepGame(state: GameState, input: InputState): GameState {
  ...
  collectLoot(state);
  checkExit(state);
  state.effects = state.effects.filter((effect) => effect.expiresAtTick > state.tick);
  state.tick += 1;
  return state;
}

export function advanceGame(state: GameState, ticks: number, input: InputState): GameState {
