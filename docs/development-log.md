...
31. Delayed the loss modal until the exact player death lock ends, preserving every death frame on the real composed page. Added deterministic target impact bursts and hurt tint, a readable Ashfang wind-up/lunge, and a Stonekin raise/slam/recoil. The one-shot profile now requires at least five distinct real raster masks, turning the readability finding into a regression gate without loosening continuity limits. Visual review accepted the new motion, after which only the combat-impact and injected-mid-action baselines affected by those intentional pixels were updated.
