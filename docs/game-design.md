...
Generated rooms receive two deliberately distinct scenery classes. Explicitly flat decals—such as scorch, blood, bones, ritual paint, and flat board fragments—are passable ground story: they sit below actors and never create invisible collision. Every raised structure, prop, and rubble pile is solid, including art that is visually presented as a decal but has height or a contact base. The raised-decal negative control proves that a raised mark cannot accidentally inherit the flat-decal pass-through policy. The first scorch mark is locked to the forge anchor so the building has a repeatable contact treatment.

Buildings and raised props have deterministic elliptical collision footprints around the part of each sprite that touches the ground. Players and monsters cannot cross a forge, tree trunk, wagon, grave marker, crate stack, cage, pillar, rubble pile, or other raised object, but axis-separated movement lets them slide along its base. Only explicitly flat decals are passable; the rift exit remains enterable. Placement, visible sprite identity, and collision all derive from the same map layout, so restoring an arbitrary state cannot move an invisible collider away from its art.
