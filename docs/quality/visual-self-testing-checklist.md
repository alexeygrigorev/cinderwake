...
- Current evidence: `tests/unit/scenery-collision.test.ts` tests “keeps opening collision contact…” (lines 130–163) and “names a blocked object…” (lines 213–259); `tests/e2e/scenery-collision.spec.ts`; terrain boundary contrast mutation in `tests/e2e/screen-contract.spec.ts` lines 819–864; and `npm run test:collision`, which records all generated and Embercross solid-role inventories (50 roles per profile), representative cardinal contacts spanning the opening, every later generated room, and the city route, friendly and hostile projectile sweeps, alpha-mask support bounds, compact per-tick contact-manifest projections, and five named mutations across desktop, phone portrait, and phone landscape. Each topology-inaccessible side is retained as an explicit skip rather than treated as a contact.
- Missing automation/next implementation: expand the representative contact target set to every retained solid role, with a reviewed topology declaration for sides that cannot be reached, and add native high-DPR capture plus independent visual review. The current evaluator already generalizes alpha-derived support comparison, profile/scenario-scoped side coverage, and invisible/offset collider mutations to every retained solid inventory entry.
