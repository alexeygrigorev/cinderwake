...
- Current evidence: exhaustive atlas audit and its controls; `scripts/assess-sequence.mjs`; `tests/visual/animation.spec.ts` tests “idle loop…” and “walk is monotonic…”; `quality/vanguard-motion-contract.v1.json` and `tests/unit/vanguard-gait-contract.test.ts` add all-facing height, alpha-bound foot/knee/torso/root landmarks, support alternation, articulation, phase-order, anchor-shift, and stretch controls; `docs/decisions/0012-raster-aware-temporal-continuity.md` and `docs/decisions/0033-vanguard-semantic-gait-promotion.md`.
