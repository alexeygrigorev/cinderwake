...
- Current evidence: `RenderManifestV1.paintQueue` is the Canvas renderer's exact scene/shadow/body/health paint plan; `tests/unit/depth-transition.test.ts` rejects actor/prop swaps, health-behind-owner, duplicate bodies, invalid queue indices, and rear-actor coverage. The `depth-transition-thorn-pillar` production route and its command tape reproduce the audited `cinder-041` behind→front crossing on desktop and 390×844; browser evidence requires a nonzero isolated actor/prop alpha-mask intersection, correct order flip, final world position, and no movement block. Effect detachment remains owned by the combat-specific `assessCombatReadability` control because only combat effects have an owner semantic.
- Missing automation/next implementation: repeat this ordered crossing for every tall occluder and record attack-on-both-sides contact frames. This row remains **Partial** pending that matrix and independent visual review.
