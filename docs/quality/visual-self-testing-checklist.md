...
- Current evidence: `tests/unit/scenery-collision.test.ts` tests “keeps opening collision contact…” (lines 130–163) and “names a blocked object…” (lines 213–259); `tests/e2e/scenery-collision.spec.ts`; terrain boundary contrast mutation in `tests/e2e/screen-contract.spec.ts` lines 819–864; and `npm run test:collision`, which records generated and Embercross solid-role inventories, representative keyboard/route contacts, friendly and hostile projectile sweeps, alpha-mask support bounds, per-tick movement manifests, and four named mutations on desktop and phone portrait.
- Missing automation/next implementation: extend the recorder’s representative contacts to all four principal sides of every solid role, including every later generated room and the complete mobile crop matrix. The current evaluator already generalizes alpha-derived support comparison and invisible/offset collider mutations to every retained solid inventory entry.
