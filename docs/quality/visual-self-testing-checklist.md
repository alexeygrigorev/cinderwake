...
- Current evidence: `npm run test:production-liveness` retains cold desktop and phone-portrait production launches for all three classes, every enabled selection/gameplay control, synchronized observer state/manifest/frame hashes, physical gesture/video logs, and abort/stall Retry/Back frames; its pure evaluator and five named mutations are exercised before the bundle is written. The focused `tests/e2e/live-player-journey.spec.ts`, `tests/e2e/physical-gesture-temporal.spec.ts`, `tests/e2e/mobile.spec.ts`, and loading recovery tests remain regression coverage.
- Missing automation/next implementation: complete the independent visual review and bind its verdict with `npm run quality:presentation:bind`; machine liveness, census bijection, recovery, and frozen transition deadlines are now retained and contractually complete.
...
- Current evidence: `npm run test:flicker && npm run capture:matrix && npm run quality:temporal:check` retains 26 sequential production sequences with command tapes, per-tick state/manifests, contact sheets, reports, desktop/phone-portrait coverage, and four matrix signals. It also imports the ordinary public-route compositor bundle: five-frame idle/move/Strike/Ability strips for Vanguard, Ranger, and Arcanist on both profiles, per-actor rAF samples/videos, and six PNG-level production-compositor mutations mapped to the named temporal detectors. The exhaustive atlas audit and its controls, `scripts/assess-sequence.mjs`, `tests/visual/animation.spec.ts`, `quality/vanguard-motion-contract.v1.json`, `tests/unit/vanguard-gait-contract.test.ts`, `docs/decisions/0012-raster-aware-temporal-continuity.md`, and `docs/decisions/0033-vanguard-semantic-gait-promotion.md` remain supporting evidence.
- Missing automation/next implementation: complete the retained presentation-run binding and independent visual review at actual play scale and normal playback speed; the portrait browser profile is observed-only and does not claim native high-refresh coverage.
...
- Required negative control: enlarge or offset the collider beyond visible alpha; remove the object sprite while retaining collision; remove blocked feedback; make an endpoint-only collision test; omit one declared principal-side contact. Each must fail a distinct signal.
- Current evidence: `tests/unit/scenery-collision.test.ts` tests “keeps opening collision contact…” (lines 130–163) and “names a blocked object…” (lines 213–259); `tests/e2e/scenery-collision.spec.ts`; terrain boundary contrast mutation in `tests/e2e/screen-contract.spec.ts` lines 819–864; and `npm run test:collision`, which records all generated and Embercross solid-role inventories (50 roles per profile), exhaustive semantic cardinal contacts for every role across the opening, later generated rooms, and city route, friendly and hostile projectile sweeps, alpha-mask support bounds, compact per-tick contact-manifest projections, and five named mutations across desktop, phone portrait, and phone landscape. Each topology-inaccessible side is retained as an explicit skip; the four all-side opening architecture roles use the validated `map-blocked-boundary` exemption. Lossless contact PNG triplets remain on a representative visual subset while every role retains semantic contact evidence.
- Missing automation/next implementation: add native high-DPR capture and independent visual review for the exhaustive contact evidence. The current evaluator generalizes alpha-derived support comparison, profile/scenario-scoped side coverage, and invisible/offset collider mutations to every retained solid inventory entry, and rejects arbitrary or malformed topology exemptions.
...
### PRES-SPRITE-009 — raster provenance and approved native text roles
...
- Machine signal and threshold: unapproved visible text count is exactly zero. Existing sprite roles, their decorations, and canvas draws require decoded local raster provenance. Native text is limited to exactly allowlisted titles and the unique semantic campaign controls/open modal journal described in `docs/testing-architecture.md`; narrative prose is at least 16 CSS px and controls at least 14 px, with 12 px reserved for explicit metadata. A title or campaign marker alone does not grant an exemption. Campaign CSS surfaces are recorded as native UI, not raster art.
- Required negative control: replace one glyph/button/health/frame/scenery role with CSS shape or plain text; mark ordinary copy as a title; forge a campaign text marker; add a CSS gradient/border/shadow decoration beside an otherwise sprite-backed element. The complete draw inventory must name each offender. Focused tests also reject undersized narrative, duplicate roots, and nonmodal journals.
- Current evidence: `npm run test:sprite-provenance` records the public selection, ordinary production launch, open campaign journal, Test Lab, win, loss, and Embercross service route at desktop and phone-portrait profiles. It retains visible-role/text/title, native text semantics and font sizes, pseudo-element, CSS-decoration, canvas-operation, manifest-draw, decoded-asset, state, and original-resolution PNG evidence, then runs four provenance signals and four named negative controls. The presentation binder attaches this bundle to `PRES-SPRITE-009` with every retained artifact hash-bound; the row remains `NEEDS_VISUAL_REVIEW` and its scenario/profile coverage remains partial.
- Missing automation/next implementation: extend the required scenario matrix whenever a new screen or conditional visual role is added, and keep the curated semantic title allowlist synchronized with that inventory. Provenance itself is machine-checked; visual composition remains covered by the style/composition rows and their independent review requirements.
...
- Current evidence: `npm run test:mobile-screen` retains portrait and landscape profile screenshots/video, hit geometry, safe-area/device projection, text metrics, synchronized state/manifest timelines, and all nine named mutations under `quality-results/mobile-screen/pres-mobile-010`; `tests/unit/mobile-screen-evidence.test.ts` exercises the pure evaluator. The presentation binder attaches this bundle to `PRES-MOBILE-010` as `NEEDS_VISUAL_REVIEW`.
- Missing automation/next implementation: physical-device artifact ingestion remains open; Playwright touch emulation cannot prove handset GPU, display, thermal, or touch-sampling behavior. Independent review of original-resolution frames and normal-speed video remains mandatory.
...
- Current evidence: environment mutations in `tests/unit/environment-composition.test.ts`; actor safe-crop audit; isolated draw signatures in sequence reports; and `npm run test:compositor`, which scans 772 nonblank actor/effect/loot cells against the versioned transparent-border contract, retains representative dark/light checkerboard pairs, compares exact production PNGs across repeated renders and a populated-state→fresh-state transition, and checks one manifest body paint per owner.
- Missing automation/next implementation: run checkerboard and isolated-mask evidence for every live-rendered actor/effect at each required device profile, add cross-cell/matte/fringe checks for every authored role, and calibrate temporal persistence thresholds from accepted effects. The current five mutations and deterministic source corpus are machine evidence, not a complete row pass.
...
- Ordered artifacts: input/aim vector; actor facing state; sprite ID, `facingBucket`, `flipX`, frame; world/screen displacement; expected-vs-actual raster hashes; ordered crops/contact sheet.
- Machine signal and threshold: deterministic vector→bank/flip mapping is exact for each actor's declared source orientation; the real isolated actor raster equals the independently rendered expected orientation; movement sign agrees with input; recovery remains on the selected bank; weapon/projectile origin aligns with authored anchor contract.
- Required negative control: select opposite bank, forget horizontal reflection, retain stale bank after turn, render the opposite raster, and mirror pose without mirrored attack origin.
- Current evidence: `npm run test:directional-bank` records desktop and phone-portrait browser evidence for Vanguard, Ranger, and Arcanist across all four cardinal movement directions, immediate opposite turns, target-directed primary and ability actions, impact, and lock-release recovery. `directional-bank-selection-v1` checks exact `spriteId`/`facingBucket`/`flipX` mapping against `art/actor-atlas-v1.json`, compares east/west canvas pixels to the expected source/mirror raster, checks target aim, pending-action origins, produced effect/projectile origins, and recovery-bank continuity; `tests/unit/directional-bank-evidence.test.ts` and the browser recorder detect seven named mutations. Each profile retains 11 synchronized semantic/raster captures and eight representative ordered PNG stages per direction under `quality-results/directional-bank/pres-facing-015/`.
- Missing automation/next implementation: bind authored weapon-anchor offsets and the remaining authored temporal scenario matrix into this row; native high-DPR capture and independent visual-agent review must still confirm pose readability at normal playback speed.
...
- Current evidence: `npm run test:camera-motion` records desktop and phone-portrait production strips for six explicit run specs: `map-edge-reversal` (approach and abrupt west/east reversals), `camera-west-south-edge` (separate west and south boundary traversals), `camera-diagonal-corner` (diagonal travel and a released stop near the north-east corner), `camera-stop-center`, `fixed-camera-open-floor-arcanist`, and `snap-camera-open-floor-arcanist`. Each profile retains synchronized camera/target timelines, full frames, contact sheets, and video; `camera-motion-continuity-v1` checks mode-specific fixed/snap behavior, both declared edge axes, and the five named mutations; the temporal camera sequence remains in the capture matrix; `tests/e2e/browser-bridge.spec.ts` covers one-update smooth follow and `cameraFollow: false`; `tests/unit/camera-projection.test.ts` covers projection bounds.
- Missing automation/next implementation: complete the independent visual review at normal playback speed.
...
- Machine signal and threshold: every expected-present entity has exactly one manifested draw; no unexplained absent sample; pixels outside the union of current draws must match the freshly rendered scene; every retained effect must match its expected geometry/owner and first-absent tick. Temporal difference/ghost thresholds have **calibration required** from accepted effects and camera motion.
- Required negative control: skip `clearRect`, draw actor twice with an offset, omit one presentation frame, retain a prior effect after despawn, mismatch an effect owner, and freeze the live renderer.
- Current evidence: `npm run test:flicker` retains four ordered deterministic compositor segments covering actor, monster, loot, projectile, and effect ownership; its effect corpus renders owned `slash`, `nova`, and `impact` geometries alongside a runtime projectile impact, records exact despawn ticks, compares a populated-state transition against a fresh reconstruction with decoded pixel residual bounds, and runs six named mutations through `tests/unit/live-compositor-evidence.test.ts` and the browser recorder. It also samples the ordinary production route through the observe-only boundary during sustained desktop movement/turning and repeated attacks, gates a real-clock desktop profile, and retains portrait DPR-3 rAF samples plus normal/slow videos and lossless action frames. The portrait profile is explicitly observed-only because this environment has not calibrated high-refresh or physical-device cadence.
- Missing automation/next implementation: bind the live flicker bundle into the retained presentation run, capture and review it on a physical high-refresh phone, and calibrate temporal ghost thresholds from accepted effects and camera motion. The desktop gate proves a live rAF loop; the observed-only portrait result is diagnostic evidence, not a high-refresh claim.
...
- Production gesture: discover the city sign/landmark, navigate to and enter the gate, return to the visible city gate to seal the rift, then approach each resident, open the merchant and buy/sell, open the tavern and eat/sleep, and open the healer and heal.
...
- Machine signal and threshold: the route landmark and gate are visible and reachable; entry and the return-gate seal complete through the production transition; the visible win outcome agrees with `run_won`; every visible service control emits its declared intent; each affordable valid action changes exactly its documented state and feedback. No injected beside-NPC fixture may stand in for the discovery route.
- Required negative control: remove the city sign, disable gate entry, disable the return-gate win, remove one service listener, and suppress one service outcome/feedback. The journey evaluator must fail each at the first missing affordance, transition, win, intent, or outcome.
- Current evidence: `npm run test:city-journey` now retains both no-bridge production-input scenarios across desktop, phone portrait, and phone landscape, with synchronized state/manifest/frame artifacts, cold wilderness-to-return-gate milestones, five successful service receipts, and all five mutations. `tests/e2e/city-production-route.spec.ts` remains a separate focused regression proof.
...
- Current evidence: `npm run test:state-replay` now records two browser replay bundles, compares both declared tick lists and every state/manifest/frame tick, verifies each recorded state and manifest hash against its serialized evidence, and runs seven required negative controls; `tests/unit/state-replay.test.ts` exercises the comparator without a browser.
