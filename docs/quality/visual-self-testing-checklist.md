...
- Current evidence: `npm run test:sprite-provenance` records the public selection, ordinary production launch, Test Lab, win, loss, and Embercross service route at desktop and phone-portrait profiles. It retains visible-role/text/title, pseudo-element, CSS-decoration, canvas-operation, manifest-draw, decoded-asset, state, and original-resolution PNG evidence, then runs the four provenance signals and three named negative controls. The presentation binder attaches this bundle to `PRES-SPRITE-009` with every retained artifact hash-bound; the row remains `NEEDS_VISUAL_REVIEW` and its scenario/profile coverage remains partial.
