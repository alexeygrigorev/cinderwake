...
3. **Player adapter (complete):** wilderness pickup writes Ashfang pelts into `city.traveler.inventory`; `GameHost.executeNearbyCityAction` preserves that inventory while copying live-player gold, health, maximum health, and tonics into the traveler, executes the atomic receipt, then mirrors those scalar fields back to the live player.
...
6. **Rendering (controlled integration complete):** the exact reviewed V3 atlas supplies the market, tavern, infirmary, open gate, road sign, and bed/food cluster. Compatible authored props build the square and service approaches. A separate audited 4 × 4 atlas gives Mara, Oren, Tess, and Ileya one declared south-facing idle row each; its four-pose loop, anchor, public bytes, and role mapping are executable contracts rather than borrowed hero art. Every city footprint is projected inside its tight sprite ink, and the gate uses two pier footprints with no center collider.
