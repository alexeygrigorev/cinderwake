...
| Idle→walk→idle transition                           | exact declared phases, east facing, and full clip banks | proves a start/stop capture did not skip either transition or loop      |
| Start/stop isolated foot anchor / raster bottom     | `≤ 0.25` / `≤ 1` logical px range                       | rejects pivot drift independently of world and camera movement          |
| Idle↔walk median visible-height difference          | `≤ 8` logical px                                        | rejects a clip-wide apparent scale pop at movement start or stop        |
...
The sequence assessor names its expectations through capture profiles: `pose`, `static-pose`, `loop`, `one-shot`, `one-shot-floating`, `death`, `anchored-motion`, `start-stop`, `projectile`, and `camera-smooth`. A profile selects relevant checks rather than pretending that an idle sprite and a projectile have identical motion obligations. Every profile still keeps semantic frame timing, transparent entity-mask evidence, and a replayable state/tape bundle. A `start-stop` command tape also carries a machine-readable clip-transition contract: exact observed phase boundaries, required semantic frames, facing, and non-relaxable grounding/height thresholds.

The shipped public matrix contains 26 deterministic reports: four directional locomotion runs; one mobile run sampled at quarter-tick interpolation; current-runtime Ashfang and Arcanist east-facing idle→walk→idle sequences on the generated opening floor; all six east-facing hero primary/ability actions; explicit north-facing Ranger and south-facing Arcanist action/recovery regressions; all three enemy attacks; enemy death through terminal pose and despawn; long projectile travel; a separate projectile hit/effect/despawn lifecycle; a full 48-tick loot-bob loop; smooth camera convergence; the wilderness-to-Embercross transition; and restored win/loss terminal presentations. The pre-won fixture proves arbitrary-state restoration and the terminal overlay, while the production city route proves reachable victory at the return gate. The start/stop gate originally rejected Ashfang at 54/66 logical pixels for idle/walk median visible height (12 px difference) and Arcanist at 104/80 (24 px difference). Declared foot-anchored rig transforms now produce 54/54 (0 px) and 104/98 (6 px) respectively, below the unchanged 8 px ceiling with zero foot-anchor and raster-bottom drift. The exhaustive atlas report additionally measures every clip for all six actors across all four cardinal facings, including registered monster ability banks that current AI does not select. Keyboard/pointer, touch controls, viewport layout, selection, generated maps, and broader state behavior remain in the Playwright/unit reports. Expand both layers with each newly shipped system, especially the roadmap Cinder Nodes, dodge, equipment, and boss content.

The start/stop assessor includes a paired detector control. A 48/49 px baseline has no phase-height mismatch; raising the paired walk-mask height measurements by 9 logical pixels fails only `startStopIdleWalkMedianVisibleHeight`. The result is emitted in every start/stop analysis rather than treating a known-red production capture as the detector proof.
