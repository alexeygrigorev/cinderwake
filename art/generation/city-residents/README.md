...
This directory keeps the generated 4 × 4 resident source behind a deterministic, reviewable preparation boundary. The exact prepared hash is registered through `art/resident-atlas-v1.json`; no raw generated bytes are loaded by the game. The raw PNG, frozen prompt, references, and generation metadata establish provenance, while `prepare.mjs` is the only permitted path from those raw bytes to the production atlas.
...
A preparation `PASS` means the raw bytes can be deterministically cut and that the resulting idle rows meet the encoded mechanical continuity rules. It does not by itself certify character identity, art direction, gameplay scale, interaction readability, or animation timing.

Runtime promotion additionally binds the exact prepared hash in
`art/resident-atlas-v1.json`, copies those bytes unchanged to
`public/assets/sprites/embercross-residents-idle-v1.png`, and requires catalog,
manifest, isolated-mask, temporal-loop, physical production-entry, and in-game
screenshot controls. The four source rows are never inferred or remapped at
runtime. Adding a resident therefore means adding one audited row and one
declarative row entry; it does not require hand-cut coordinates in the game
renderer.
