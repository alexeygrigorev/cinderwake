...
const splice = async (family) => {
  const base = await sharp(path.resolve(root, manifest.base[family]))
    .ensureAlpha()
    .raw()
    .toBuffer();
  ...
  for (const cell of changed)
    for (let y = 0; y < 256; y++) {
      const offset =
        ((Math.floor(cell / 4) * 256 + y) * 1024 + (cell % 4) * 256) * 4;
  ...
  await sharp(mixed, { raw: { width: 1024, height: 1024, channels: 4 } })
    .png()
    .toFile(file);
  return {
    family,
